                    ...
                    <div style="flex-grow: 1;">
                        ...
                        <?php if (!empty($person['contacts'])) : ?>
                            ...
                            foreach ($person['contacts'] as $index => $contact) {
                                // Move displayed_contacts check inside the contact posts loop
                                $displayed_contacts = [];
                                if (!empty($contact_posts)) {
                                    foreach ($contact_posts as $post) {
                                        $displayed_contacts = get_post_meta($post->ID, 'displayed_contacts', true) ?: [];
                                    }
                                }

                                // Check if the current contact index is in $displayed_contacts
                                if (!empty($displayed_contacts) && !in_array($index, $displayed_contacts)) {
