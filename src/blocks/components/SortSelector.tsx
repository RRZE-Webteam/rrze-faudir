...
const sortLabelMap: Record<SortValue, string> = {
  ...
  "honorificprefix, familyName": __(
    "Academic title, then family name",
    "rrze-faudir",
  ),
  role: __("Head of Department first", "rrze-faudir"),
  "role, honorificprefix": __(
    "Head of Department first, then academic title",
    "rrze-faudir",
  ),
...
function buildSortOptions(): Array<{ value: SortValue; label: string }> {
  return allowedSortValues.map((value) => {
    return {
      value: value,
...
export default function SortSelector({
  ...
  const rawOrder =
    typeof attributes.order === "string" ? attributes.order : "asc";
  ...
  return (
    <>
      ...
      <SelectControl
        ...
        help={__(
          "Applies to all criteria unless you override it per criterion below.",
          "rrze-faudir",
        )}
      />
