...
export default function Edit({ attributes, setAttributes }: EditProps) {
  ...
  const visibleFields =
    Array.isArray(visibleFieldsAttr) && visibleFieldsAttr.length > 0
      ? visibleFieldsAttr
  ...
  useEffect(() => {
    ...
    apiFetch<OrganizationResponseProps>({
      ...
      .then((response: OrganizationResponseProps) => {
        ...
        const nextOfficeHours: OfficeHour[] = Array.isArray(
        ...
        const nextTextContent = content.filter((entry) => {
  ...
  const dataViewDataBuffer = useMemo<ServiceDataRow[]>(() => {
  ...
  const hasAnyContact = ["phone", "mail", "url"].some((fieldId) => {
    return (
      isFieldVisible(fieldId) && Boolean(contact[fieldId as keyof ContactData])
    );
  });

  const hasAddress = ["street", "zip", "city"].some((fieldId) => {
    return (
      isFieldVisible(fieldId) && Boolean(contact[fieldId as keyof ContactData])
    );
  });
    ...
    return (
      <div {...blockProps}>
        <Placeholder
          ...
          instructions={__(
            "Insert your FAUdir Folder/Org Id to display service information.",
            "rrze-faudir",
          )}
        >
  ...
  return (
    <div {...blockProps}>
      ...
      {modalDataView && (
        ...
        >
          <Notice
            isDismissible={false}
            spokenMessage={__(
              "Please be aware, that all data displayed within the service block is in sync with the Portal FAUdir. You cannot change contact details from within your web page.",
              "rrze-faudir",
            )}
            status="info"
          >
            {__(
              "The data displayed below is in sync with FAUdir and cannot be changed from within your website. Contact data can only be edited within the FAUdir Portal.",
              "rrze-faudir",
            )}
          </Notice>
      ...
      <InspectorControls>
        ...
        <PanelBody
          title={__("Available data", "rrze-faudir")}
          initialOpen={false}
        >
          <Button
            variant="tertiary"
        ...
        <div className="rrze-elements-blocks_service__info">
          {isFieldVisible("name") && organizationName && (
            <header className="rrze-elements-blocks_service__meta_headline">
              ...
              {isFieldVisible("text") && description && <p>{description}</p>}
            </header>
          )}

          {hasAddress && (
            <section
              className="rrze-elements-blocks_service__information"
              aria-labelledby="addr-h"
            >
            ...
            <section aria-labelledby="hours-h">
              ...
              <ul className="list-icons">
                {formattedOfficeHours.map((entry, index) => {
                  return <li key={`office-hour-${index}`}>{entry}</li>;
                })}
              </ul>
            </section>
          )}
