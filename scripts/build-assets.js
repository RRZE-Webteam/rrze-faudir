...
function buildJs(mode, watch) {
    ...
    return esbuild.build({
        entryPoints: [path.join('src', 'js', 'admin', 'admin.js')],
        bundle: true,
        sourcemap: isProd ? false : true,
        minify: isProd ? true : false,
        format: 'iife',
        target: ['es2018'],
        outfile: path.join('assets', 'js', 'rrze-faudir-admin.js')
    }).then(function () {
        return true;
    });
}
...
function buildCss(mode) {
    var isProd = mode === 'prod';
    var outFile = path.join('assets', 'css', 'rrze-faudir.css');
    var result = sass.compile(path.join('src', 'scss', 'rrze-faudir.scss'), {
...
function runWatch(mode) {
    ...
    return jsCtxPromise.then(function (ctx) {
        return ctx.watch().then(function () {
            sass.compileAsync(path.join('src', 'scss', 'rrze-faudir.scss'), {
                style: 'expanded',
                sourceMap: true,
                sourceMapIncludeSources: true
                ...
                sass.watch(path.join('src', 'scss', 'rrze-faudir.scss'), function () {
                    sass.compileAsync(path.join('src', 'scss', 'rrze-faudir.scss'), {
                        style: 'expanded',
                        sourceMap: true,
                        sourceMapIncludeSources: true
                ...
                console.log('[rrze-faudir] watching JS+SCSS...');
                return true;
            });
        });
    });
